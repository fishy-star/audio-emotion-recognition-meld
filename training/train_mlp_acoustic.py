...
# Load acoustic flat-aggregate features (192,) -- mean+std over the GFCC/MFCC/
# F0/chroma/spectral/ZCR/delta feature set, already StandardScaler-normalized
# (fit on train only) by utils/extract_acoustic_features.py.

X_train = np.load(
    os.path.join(EMBED_DIR, "train_acoustic_flat.npy")
)

X_dev = np.load(
    os.path.join(EMBED_DIR, "dev_acoustic_flat.npy")
)

print("Train acoustic features:", X_train.shape)
print("Dev acoustic features:", X_dev.shape)
...
# Encode labels. Fit a NEW encoder and save it under outputs/mlp_acoustic --
...
best_path = os.path.join(CHECKPOINT_DIR, "mlp_acoustic_best.pt")
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        output = model(X_dev_tensor.to(device))
