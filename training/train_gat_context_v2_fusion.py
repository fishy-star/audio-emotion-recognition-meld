...
OUTPUT_DIR = os.path.join(config.OUTPUT_DIR, "gat_context_v2_fusion")
CHECKPOINT_DIR = os.path.join(config.CHECKPOINT_DIR, "gat_context_v2_fusion")
...
# DropEdge rate, unchanged from train_gat_context_v2.py
DROP_EDGE_P = 0.15
...
best_path = os.path.join(CHECKPOINT_DIR, "gat_context_v2_fusion_best.pt")
...
for epoch in range(epochs):
    ...
    for idx in train_sampler:
        ...
        # DropEdge: randomly drop a fraction of non-self-loop edges each
        # step, regularizing against overfitting to specific dialogue
        # structures.
        edge_index = drop_edges(graph.edge_index, p=DROP_EDGE_P, training=True)

        output = model(graph.x, graph.x_acoustic, edge_index)
